...
/* Gợi ý các kho khác đang còn hàng (hiện khi kho đang chọn không đủ tồn) */
function vfStockHint(pid, whId){
  var others=[];
...
function vfBindSearch(){
  ...
  inp.addEventListener('input', function(){
    ...
    dd.innerHTML=list.length?list.map(function(p){
      var needSt=vfNeedsStock(VF.type);
      var st=needSt?getStock(whId,p.id):totalStock(p.id);
      var tot=totalStock(p.id);
      var extra=needSt&&Math.abs(tot-st)>1e-9?' <span style="color:var(--muted2)">· tổng '+fmtQty(tot)+'</span>':'';
      return '<div class="dd-item" onmousedown="vfPick(\''+p.id+'\')"><div><b>'+esc(p.name)+'</b> <span class="muted small">· '+esc(p.sku)+(p.brand?' · '+esc(p.brand):'')+'</span></div><div class="muted small">'+(needSt?'Tồn tại kho: ':'Tồn: ')+fmtQty(st)+' '+esc(p.unit)+extra+'</div></div>';
...
function vfUpdateCalc(){
  ...
  VF.lines.forEach(function(l,i){
    ...
    if(vfNeedsStock(VF.type)){
      ...
      var over=l.qty>st;
      var qEl=$('#vfl-qty-'+i);
      if(qEl) qEl.style.borderColor = over ? 'var(--red)' : '';
      var stEl=$('#vfl-st-'+i);
      if(stEl) stEl.className=over?'num-neg':'muted';
      var hEl=$('#vfl-hint-'+i);
      if(hEl) hEl.classList.toggle('hidden', !over);
    }
  });
